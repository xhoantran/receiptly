...
async function runTool(name: string, input: any): Promise<unknown> {
  switch (name) {
    case "query_transactions":
      return listTransactions({ merchant: input.merchant, from: input.from, to: input.to, limit: input.limit ?? 50 });
    case "merchant_summary":
      return merchantSummary();
    case "spending_by_item":
      return (await spendingByItem({ like: input.like, from: input.from, to: input.to })).slice(0, 40);
    case "price_history":
      return priceHistory(input.item);
    case "savings_summary":
      return savingsSummary({ from: input.from, to: input.to });
    case "spending_by_category":
      return spendingByCategory();
    case "products":
      return (await canonicalProducts({ category: input.category, limit: input.limit ?? 40 }));
    default:
