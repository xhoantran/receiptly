 ...
 * the same engine the local CLI flow (session.ts) uses; here it runs headless.
 *
...
export type OpenContextOptions = {
  ...
  viewport?: Viewport;
  /** Run headless. Defaults to true (the live-view streams screenshots). */
  headless?: boolean;
};
...
export class LocalBrowserProvider implements BrowserProvider {
  async openContext(opts: OpenContextOptions = {}): Promise<OpenedContext> {
    const browser = await webkit.launch({ headless: opts.headless ?? true });
...
/**
 * The active provider. The hosted control plane swaps this for a CDP/remote
 * provider (a managed pool of WebKit browsers) — same interface, so callers in
 * scrape.ts and the live-view worker are unaffected.
 */
