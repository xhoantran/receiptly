...
export async function resolveProducts(
  ...
  // Only this user's line items are (re)resolved; the product catalog they
  // resolve INTO is global/shared.
  const items = await db
    ...
    .where(
      opts.force
        ...
        : and(eq(receiptItems.userId, userId), isNull(receiptItems.productId))
    );
