...
async function scrapeOne(key: string) {
  ...
  const r = await ingest(key, receipts);
  console.log(
...
async function main() {
  switch (cmd) {
    case "init": {
      ...
      for (const c of connectors) await upsertConnector({ key: c.key, displayName: c.displayName, mode: c.mode });
      console.log(`Seeded ${connectors.length} connectors. (Run \`npm run db:push\` first for a fresh database.)`);
      break;
    }
    case "link": {
    ...
    case "scrape": {
      ...
      const work = (await coverageStats())
    ...
    case "tx": {
      ...
      const rows = await merchantSummary();
    ...
    case "resolve": {
      const { resolved, created } = await resolveProducts({ force: flags.has("--force") });
