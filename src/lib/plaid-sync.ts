...
/** Pull new transactions for every linked Plaid item into the DB. */
export async function syncAll(): Promise<{ items: number; added: number }> {
  ...
  for (const it of items) {
    ...
    await upsertTransactions(res.added, (t) => findConnector(t)?.key ?? null);
    if (res.cursor) await updateCursor(it.itemId, res.cursor);
